...
def svrRemoveFeature(input_data,features_interest):
    ...
    all_features = ['pdop','ndop','nr_used_measurements','correction_covariance','correction_state','correction_state_ENU','difference_ENU','innovation_ENU','innovation','prediction_covariance','prediction_state','prediction_state_ENU','cno','constellation','tracking_type','elevation','azimuth','lsq_residual','multipath','raim','cycle_slip','sky']
...
def svrDataVectors(input_data,target,all_features_interest):
    ...
    for i in range(0,len(ind)):
        ...
        for j in all_features_interest:
            ...
            if j == 'pdop' or j == 'ndop' or j == 'nr_used_measurements' or j == 'sky':
